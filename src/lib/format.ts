...
/** Prisma hands back Date; a serialised payload hands back a string. */
...
export function priceRangeLabel(
  ticketTypes: ReadonlyArray<{ priceCents: number }> | undefined | null
): string | null {
...
export function formatEventDate(startsAt: DateLike): string {
  return format(new Date(startsAt), 'EEE d MMM');
}

export function formatEventDateLong(startsAt: DateLike): string {
 ...
 */
export function formatEventWindow(startsAt: DateLike, endsAt: DateLike): string {
...
export function relativeToNow(date: DateLike): string {
...
export function calendarParts(startsAt: DateLike) {
