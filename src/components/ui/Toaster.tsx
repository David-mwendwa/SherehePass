...
export function toast(
  message: string,
  {
    tone = 'success',
    duration = 4000,
  }: { tone?: ToastTone; duration?: number } = {}
): void {
...
export function Toaster() {
  ...
  const subscribe = useCallback<Listener>((event) => {
    setItems((current) =>
      event.type === 'add'
        ? [...current, event.entry]
  ...
  useEffect(() => {
    ...
    // Braces matter: `Set.delete` returns a boolean, and an effect cleanup that
    // returns a value is a type error (and, in React, a footgun — a returned
    // non-function is silently ignored).
    return () => {
