 ...
 *
 * This parses a querystring the browser controls, so every part is treated as
 * possibly absent — `"abc"` with no colon, `":"` with no id, a trailing comma.
 * `noUncheckedIndexedAccess` is what forces that to be written down rather than
 * assumed; the old JS version read `undefined` into `ticketTypeId` and let it
 * travel as far as the database query.
 */
function parseLines(raw: string): PurchaseLine[] {
  return raw
    .split(',')
    .filter(Boolean)
    .flatMap<PurchaseLine>((part) => {
...
export type CheckoutFields = 'name' | 'email' | 'phone' | 'lines' | 'method' | 'eventId';

export async function checkoutAction(
  _prevState: FormState<CheckoutFields>,
  formData: FormData
): Promise<FormState<CheckoutFields>> {
  ...
  if (!parsed.success) {
    return { errors: fieldErrorsFrom<CheckoutFields>(parsed.error) };
  }
